...
void Bus::buildbus() {
	...
	for (int j = 0; j < N; j++) {//有N班车，分别输入每班车的发车时间和收车时间，以此计算每班车在每个站点的到达和离开时间
...
void Bus::show()
{
	...
	for (int j = 0; j < N; j++) {
		...
		cout << "————————————————------—" << endl;
		for (int k = 1; k < N + 1; k++) {
			cout << "停车点" << k << "到达时间    |" << "     "  << bs.bus[i].endtime[j][k] << endl;
			cout << "———————————————----——--" << endl;
			cout << "停车点" << k << "离开时间    |" << "     "  << bs.bus[i].startime[j][k] << endl;
			cout << "——————————————----———--" << endl;
		}
...
int bus_data::select_bus(int j, int k, int s) {//车类对象数组下标，车类班次，以及下车人数 
...
bus_data::~bus_data() {
	...
	for (int j = 0; j < i; j++) {
		file.write((char*)&bus[j], sizeof(bus[j]));
	}//重新将数组信息写入文件 
	file.close();
}//析构函数（结束时写入刷新后的数据）
